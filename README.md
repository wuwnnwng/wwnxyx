...
平台审核通过后会提示「代码未做加固保护」。这不是拒审，但下次上传要用微信官方《游戏深度保护》插件，否则仍会收到同样提示。

1. 微信开发者工具需 **1.06.2401020** 或更高版本
2. 打开 **工具 → 插件 → 编辑器扩展**，安装 **游戏深度保护**（也可在扩展里搜 `codeFortify`）
3. 本仓库已放好 `code.fortify.config.json`，列出全部玩法 JS，混淆等级为高，并开启水印
4. 安装插件后，在插件面板点 **全部加固**，或直接 **编译 / 上传**：配置文件存在时会自动加固
5. 加固完成后务必在模拟器和真机各玩一遍（首页、闯关、无尽、广告）再上传
6. 加固会改写 JS 并生成 `.js.map`。源码请以 Git 为准；上传前用插件加固即可。不要改已经加固过的文件。`.js.map` 建议一并提审，不计入包体积

说明见：[游戏深度保护插件](https://developers.weixin.qq.com/minigame/dev/devtools/codeprotect)
